...
export const CustomFrontMatter: QuartzTransformerPlugin<Partial<Options>> = (userOpts) => {
      ...
      return [
        ...
        () => {
          return (_, file) => {
            ...
            // Handle permalinks (single string or array of strings) WITHOUT adding them as aliases
            if (data.permalink != null) {
              if (Array.isArray(data.permalink)) {
                // Handle array of permalinks - use first one
                const link = data.permalink[0]
                if (typeof link === "string" && link.toString() !== "") {
                  data.permalink = link.toString() as FullSlug
                }
